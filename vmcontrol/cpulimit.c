	...
	while(1) {
		...
		while ((dit = readdir(dip)) != NULL) {
			...
			if (pid>0) {
				...
				if (size>0) {
					int found=0;
					if (process[0]=='/' && strncmp(exepath,process,size)==0 && ((size_t)size)==strlen(process)) {
	...
	int pid, char *exe, char *path) 
{
	...
	if( pid > 0 ) pid_ok = 1;
	if( exe != NULL || path != NULL ) process_ok = 1;
...
int cpulimit_bypid(int verbose_setting, int lazy_setting, int perclimit, int pid) {
    return cpulimit_entry(verbose_setting, lazy_setting, perclimit, pid, NULL, NULL);
}

int cpulimit_byexe(int verbose_setting, int lazy_setting, int perclimit, char *exe) {
    return cpulimit_entry(verbose_setting, lazy_setting, perclimit, -1, exe, NULL);
}

int cpulimit_bypath(int verbose_setting, int lazy_setting, int perclimit, char *path) {
    return cpulimit_entry(verbose_setting, lazy_setting, perclimit, -1, NULL, path);
}
